  ...
  then
    ...
    then
      ...
      then
        ...
        echo -e "\nWhat time would you line your $SERV_NAME, $CUSTOMER_NAME?"
        read SERVICE_TIME
        CUSTOMER_ID=$($PSQL "SELECT customer_id FROM customers WHERE phone = '$CUSTOMER_PHONE' ")
        if [[ $CUSTOMER_ID ]]
        then
          INSERT_SERV_TIME=$($PSQL "INSERT INTO appointments(customer_id, service_id, time) VALUES($CUSTOMER_ID, $SERVICE_ID_SELECTED, '$SERVICE_TIME')")
          if [[ $INSERT_SERV_TIME ]]
          then
